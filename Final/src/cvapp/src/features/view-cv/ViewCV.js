...
export function ViewCV() {
  ...
  const generatePdf = (name) => {
    ...
    html2canvas(cv, { backgroundColor: '#ffffff', scale: 4, scrollY: -window.scrollY }).then(function (canvas) {
      ...
      dispatch(addDownload({ identifier }))
  ...
  return (
    <div className='view-cv-container'>
      <div className='header'>
        ...
        <Button size='small' icon={<DownloadOutlined />} ghost onClick={handleDownload} loading={downloading}>Tải xuống</Button>
      </div>
      <div style={{ backgroundColor: '#333', padding: '100px 0 50px' }}>
        <div style={{ fontFamily, fontSize: fontSize + 'pt', lineHeight }}>
          <TemplateComponent viewMode={true} uploadImage={() => true} updateRating={() => true} info={info} />
        </div>
      </div>
    </div>
  )
